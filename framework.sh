...
function fix_ingress() {
  ...
  orig_coll_url=$(grep host sysdigcloud/openshift/openshift-collector-router.yaml | cut -d ':' -f2)
  ...
  sed -i -e "s/$orig_coll_url/$collector_url/g" sysdigcloud/openshift/openshift-collector-router.yaml
  }
...
function deploy_watchtower_v2() {
  ...
  broadcast 'g' "Deploying V2 Dashboards"
  for raw_json in watchtower/dashboards_v2.bash/*.json; do
      pretty_json=`cat $raw_json`
      ...
      curl -kX POST \
	   ...
	   -H 'X-Sysdig-Product: SDC' \
           -H "Content-Type: application/json" \
           --output /dev/null --show-error --fail --silent \
           "$api_url"/api/v2/dashboards
  done
  
  broadcast 'w' "Watchtower Dashboards V2 Deployed....."
  }
